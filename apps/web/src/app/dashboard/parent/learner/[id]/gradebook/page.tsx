...
const SUBJECT_COLORS: Record<string, string> = {
  Mathematics: "hsl(var(--visual-primary))",
  "English Language Arts": "hsl(var(--visual-science))",
  Science: "hsl(var(--visual-sel))",
  "History & Social Studies": "hsl(var(--visual-math))",
  "Coding & Computer Science": "hsl(var(--visual-reading))",
  "Speech & Language": "hsl(var(--visual-reading))",
  "Social-Emotional Learning": "hsl(var(--visual-primary))",
};
          ...
          {Object.keys(subjectGroups).length === 0 ? (
            ...
            Object.entries(subjectGroups).map(([subject, entries]) => (
              <div key={subject} className="vi-card p-6">
                <h3 className="font-heading font-bold text-lg mb-4" style={{ color: SUBJECT_COLORS[subject] || "hsl(var(--visual-primary))" }}>
                  {subject}
                </h3>
                      ...
                      <div className="flex-1 h-4 bg-[hsl(var(--visual-surface-soft))] rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all duration-500"
                          style={{
                            ...
                            backgroundColor: SUBJECT_COLORS[subject] || "hsl(var(--visual-primary))",
                          }} />
                      </div>
                      <span className="text-sm font-bold w-12 text-right" style={{ color: SUBJECT_COLORS[subject] || "hsl(var(--visual-primary))" }}>
              ...
              return (
                <div key={path.id} className="vi-card p-6">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-heading font-bold text-lg" style={{ color: SUBJECT_COLORS[path.subject] || "hsl(var(--visual-primary))" }}>
                      {path.subject}
                    </h3>
                    <span className="text-sm font-bold" style={{ color: SUBJECT_COLORS[path.subject] || "hsl(var(--visual-primary))" }}>
                      {completed}/{total} {t("topics")}
                    </span>
                  </div>
                  <div className="h-3 bg-[hsl(var(--visual-surface-soft))] rounded-full overflow-hidden mb-4">
                    <div className="h-full rounded-full transition-all duration-500"
                      style={{ width: `${progress}%`, backgroundColor: SUBJECT_COLORS[path.subject] || "hsl(var(--visual-primary))" }} />
                  </div>
