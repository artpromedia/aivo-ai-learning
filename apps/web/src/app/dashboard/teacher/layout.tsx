...
export default function TeacherLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && !["TEACHER", "PLATFORM_ADMIN"].includes(user.role)) router.push("/");
  }, [user, loading, router]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      <SkipLink />
      <aside className={`${collapsed ? "w-16" : "w-64"} bg-white border-r border-slate-200 flex flex-col transition-all duration-200 flex-shrink-0`} role="navigation" aria-label="Teacher sidebar">
          ...
          {!collapsed && (
            <Link href="/dashboard/teacher" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-600 font-semibold uppercase tracking-wider">Teacher</p>
              </div>
            </Link>
          )}
