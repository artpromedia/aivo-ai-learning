    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-slate-500 font-semibold">Tutor not found</p>
          <Link href="/dashboard/learner/tutors" className="text-primary hover:underline text-sm mt-2 inline-block">← Back to Tutors</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div className="max-w-3xl mx-auto px-6 py-8">
        <Link href="/dashboard/learner/tutors" className="text-sm text-primary hover:underline font-semibold mb-6 inline-block">← Back to Tutors</Link>
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                <p className="text-xs text-slate-400 font-semibold uppercase mb-1">Subject</p>
                <p className="text-sm font-semibold text-slate-900">{tutor.domain}</p>
              </div>
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-100">
                <p className="text-xs text-slate-400 font-semibold uppercase mb-1">Sessions</p>
            ...
            {isActive ? (
              <Link href={`/dashboard/learner/lesson/${tutorKey}`}
                ...
                Start Lesson with {tutor.name}
              </Link>
            ) : (
              <div className="text-center py-6">
                <p className="text-slate-500 font-medium mb-2">This tutor is not included in your current plan.</p>
                <p className="text-sm text-slate-400">Ask your parent to add {tutor.name} from the Tutor Store.</p>
              </div>
            )}
          </div>
        </div>
