...
export default function CaregiverLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && user && !["CAREGIVER", "PLATFORM_ADMIN"].includes(user.role)) router.push("/");
  }, [user, loading, router]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      <SkipLink />
      <aside className={`${collapsed ? "w-16" : "w-64"} bg-white border-r border-slate-200 flex flex-col transition-all duration-200 flex-shrink-0`} role="navigation" aria-label="Caregiver sidebar">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          {!collapsed && (
            <Link href="/dashboard/caregiver" className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-600 font-semibold uppercase tracking-wider">Caregiver</p>
              </div>
            </Link>
          )}
