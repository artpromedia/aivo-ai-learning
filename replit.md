...
- **District Admin Dashboard** (`/dashboard/district`): 7 pages — Overview (metrics + action cards + recent learners), Schools, Learners, Staff & Teachers, Analytics & Reports (level distribution + role charts), Usage & Limits (account + AI usage bars), Settings (org details + preferences). White sidebar with violet accent. Based on legacy aivo-v5 district-admin-web patterns (tenant management, usage limits, audit logs). Accessible by DISTRICT_ADMIN and PLATFORM_ADMIN roles.
