...
- **The Awakening Sequence**: Cinematic 7-phase animation (25s STANDARD, 15s LOW_VERBAL, 10s NON_VERBAL, parent-only PRE_SYMBOLIC) that plays after Discovery Adventure when brain clone is created. Phases: Gathering (orbs drift from edges), Convergence (orbs orbit/merge), Formation (brain sphere forms with unique color signature), Memories (flashback moments from assessment), Awakening (sphere brightens + light wave), Reveal (tutor narration + badge), Welcome Home (transition button). Colors personalized from `visual_identity` stored in brain_states JSONB (primaryHue, secondaryHues, pulseRate, growthParticles, memoryBank). Canvas-based particle system runs at 60fps. BrainSphere reusable component at `apps/web/src/components/brain/BrainSphere.tsx`. Parent brain-review page shows matching sphere + strengths-first profile framing.
