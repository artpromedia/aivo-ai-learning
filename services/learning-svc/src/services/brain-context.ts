/**
 * Brain context normalization helper for learning-svc.
 *
 * The brain-svc returns the learner profile in several shapes:
 *   - GET /api/brain/{learnerId}          → the brain record directly (camelCase)
 *   - GET /api/brain/{learnerId}/context  → a joined object with brainState,
 *                                           learner, sensoryProfile, iepGoals, etc.
 *   - some callers historically returned { state: {...} } (snake_case).
 *
 * Callers must never silently see `{}` when one of those richer shapes is
 * returned. This module normalises every known shape into a single
 * NormalizedBrainContext that exposes both camelCase and snake_case
 * fields so TypeScript callers and Python AI services can both use it.
 */
import { createLogger } from "@aivo/observability";

const logger = createLogger("learning-svc.brain-context");
...
/**
 * Detect which response shape the brain-svc returned. Detection is
 * structural rather than version-tagged so the helper remains forward
 * compatible if brain-svc adds new top-level keys.
 */
function detectShape(raw: Record<string, unknown>): BrainContextSourceShape {
  if (isPlainObject(raw.brainState) || Array.isArray(raw.iepGoals) || isPlainObject(raw.functioningLevel) || isPlainObject(raw.sensoryProfile)) {
    return "context";
  }
  ...
  if (
    "mastery_levels" in raw ||
    "masteryLevels" in raw ||
    "active_accommodations" in raw ||
    "activeAccommodations" in raw ||
    "functioning_level_profile" in raw ||
    "functioningLevelProfile" in raw
  ) {
    return "direct";
  }
  return "direct";
...
export function normalizeBrainContext(raw: unknown): NormalizedBrainContext {
  ...
  // Resolve the "brain record" portion based on shape so we can pull the
  // mastery/accommodations/etc. fields from a consistent place.
  ...
  // Learner block: present on /context responses or built from direct
  // brain record fields when the caller only hit /api/brain/{id}.
...
/**
 * Fetch and normalize the learner Brain context. Prefers /context (richer
 * joined record) and falls back to the direct brain record at /api/brain/{id}
 * if /context is unavailable. Always returns a NormalizedBrainContext —
 * never an unwrapped `{}`.
 */
export async function fetchBrainContext(
  ...
  const service = opts.service ?? "learning-svc";
